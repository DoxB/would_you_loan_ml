...
def train_model_with_params(params, **context):
    ...
    model_path = os.path.join(MODEL_PATH, f"model_n_{params['n_estimators']}_depth_{params['max_depth']}_split_{params['min_samples_split']}.pkl")
...
with DAG(
    ...
    description='랜덤 포레스트 하이퍼파라미터 튜닝 파이프라인',
    ...
    for params in itertools.product(param_grid['n_estimators'], param_grid['max_depth'], param_grid['min_samples_split']):
        ...
        train_task = PythonOperator(
            task_id=f'train_model_n_{params[0]}_depth_{params[1]}_split_{params[2]}',
